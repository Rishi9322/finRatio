...
import { calculateNetWorkingCapital } from "@/lib/financialCalculations"
import { Loader2, Calculator, Save } from "lucide-react"

const schema = z.object({
  currentAssets: z.number().min(0, "Must be >= 0"),
  currentLiabilities: z.number().min(0, "Must be >= 0"),
})

type Inputs = z.infer<typeof schema>

export default function NWCCalculator() {
  const { register, control, formState: { errors } } = useForm<Inputs>({
    resolver: zodResolver(schema),
    defaultValues: { currentAssets: 3500000, currentLiabilities: 2000000 },
  })

  const formValues = useWatch({ control }) as Inputs
  const [results, setResults] = useState<{ nwc: number, error: string | null } | null>(null)
  const [isSaving, setIsSaving] = useState(false)
  const [saveMessage, setSaveMessage] = useState("")

  const handleCalculate = () => {
    if (Object.keys(errors).length > 0) return
    const res = calculateNetWorkingCapital(formValues.currentAssets, formValues.currentLiabilities)
    setResults(res)
  }
    ...
    try {
      const response = await fetch("/api/calculations", {
        ...
        body: JSON.stringify({
          ...
          inputs: formValues,
          results: { netBenefit: results.nwc },
        }),
      })
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-6xl">
        <div className="mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-400 max-w-3xl">
            Evaluate operating liquidity and short-term financial health.
          </p>
        </div>
            ...
            {!results ? (
              ...
              <div className="space-y-6 flex-1">
                <div className="p-6 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Net Working Capital</p>
                  <p className="text-4xl font-bold text-slate-900 dark:text-white">₹{results.nwc.toLocaleString()}</p>
                </div>
