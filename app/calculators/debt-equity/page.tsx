...
import { calculateDebtEquity, interpretDebtEquity } from "@/lib/financialCalculations"
...
export default function DebtEquityCalculator() {
  const { register, control, formState: { errors } } = useForm<Inputs>({
    ...
    defaultValues: { totalDebt: 1000000, totalEquity: 500000 },
  })
  ...
  const handleCalculate = () => {
    ...
    const res = calculateDebtEquity(formValues.totalDebt, formValues.totalEquity)
    setResults(res)
    if (res.error === null) {
      setInterpretation(interpretDebtEquity(res.ratio))
    }
  }
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
            ...
            <form className="space-y-6">
              ...
              <div>
                ...
                <input
                  type="number"
                  {...register("totalEquity", { valueAsNumber: true })}
                  className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition-colors dark:text-white"
                />
            ...
            {!results ? (
              ...
              <div className="space-y-6 flex-1">
                <div className="p-6 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Debt Equity Ratio</p>
