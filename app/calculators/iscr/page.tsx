...
import { calculateISCR } from "@/lib/financialCalculations"
import { Loader2, Calculator, Save } from "lucide-react"

const schema = z.object({
  ebit: z.number().min(0, "Must be >= 0"),
  interestExpense: z.number().min(1, "Must be > 0"),
})

type Inputs = z.infer<typeof schema>

export default function ISCRCalculator() {
  const { register, control, formState: { errors } } = useForm<Inputs>({
    resolver: zodResolver(schema),
    defaultValues: { ebit: 2000000, interestExpense: 500000 },
  })
  ...
  const handleCalculate = () => {
    ...
    const res = calculateISCR(formValues.ebit, formValues.interestExpense)
    setResults(res)
  }
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-6xl">
        <div className="mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-400 max-w-3xl">
            Evaluate how easily a company can pay interest on its outstanding debt.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
            ...
            <form className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">EBIT (₹)</label>
                <input type="number" {...register("ebit", { valueAsNumber: true })} className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all" />
                {errors.ebit && <p className="mt-1 text-sm text-red-500">{errors.ebit.message}</p>}
              </div>

              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Interest Expense (₹)</label>
                <input type="number" {...register("interestExpense", { valueAsNumber: true })} className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all" />
                {errors.interestExpense && <p className="mt-1 text-sm text-red-500">{errors.interestExpense.message}</p>}
              </div>
            ...
            {!results ? (
              ...
              <div className="space-y-6 flex-1">
                <div className="p-6 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">ISCR</p>
